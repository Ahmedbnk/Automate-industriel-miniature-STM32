...
void lcd_write_data(LCD_HandleTypeDef *lcd, char data)
{
    ...
    HAL_I2C_Master_Transmit(lcd->hi2c, lcd->address << 1, buffer, 1, 100);
}
...
void LCD_PrintInt(LCD_HandleTypeDef *lcd, int num)
{
    char buffer[16];
...
void LCD_PrintFloat(LCD_HandleTypeDef *lcd, float num, int decimal_places)
{
    char buffer[20];
...
int main(void)
{
  ...
  lcd.hi2c = &hi2c1;   // your I2C instance (check CubeMX)
  lcd.address = 0x27;  // most common LCD address
  /* USER CODE BEGIN 2 */
